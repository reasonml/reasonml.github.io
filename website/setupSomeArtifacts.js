...
const getBsbVersion = 'bsb -v';
const bsbVersionOutput = childProcess.execSync(getBsbVersion);
const bsbVersion = bsbVersionOutput.toString().trim();

const getBsJsVersion = "sed -nr 's/^.*Generated by BUCKLESCRIPT VERSION ([0-9]+\.[0-9]+\.[0-9]+).*$/\\1/p' " + path.join(playgroundDir, 'bs', 'exports.js');
const bsJsVersionOutput = childProcess.execSync(getBsJsVersion);
const bsJsVersion = bsJsVersionOutput.toString().trim();

if (bsbVersion !== bsJsVersion) {
  console.log(`
** Version mismatch between BuckleScript compiler (${bsJsVersion}) and bs-platform (${bsbVersion}).`);
  process.exit(1);
}
...
const reactFile = path.join(__dirname, 'node_modules', 'react', 'dist', 'react.min.js');
...
reasonReactDirFiles.forEach(file => {
  const exposedRequireName = path.join('stdlib', lowercaseFirstLetter(path.basename(file, '.js')));
