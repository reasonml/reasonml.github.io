...
export default class Try extends Component {
    ...
    componentDidMount() {
        ...
        this.iframe.contentWindow.console = {
            ...
            error: (...items) => {
                this.setState(state => ({...state, output: state.output.concat({type: 'error', contents: items})}))
            },
            warn: (...items) => {
                this.setState(state => ({...state, output: state.output.concat({type: 'warn', contents: items})}))
            }
        }
    ...
    updateReason = (reason) => {
        ...
        if (converted[0] !== 'REtoML') {
            this.err = setTimeout(() => this.setState({
                reasonError: converted[1],
            }), errorTimeout)
            this.setState({reason, reasonError: null, bsError: null, ocamlError: null, jsIsLatest: false})
            return
        }
    ...
    updateOCaml = (ocaml) => {
        ...
        if (converted[0] !== 'MLtoRE') {
            ...
            this.setState({ocaml, reasonError: null, bsError: null, ocamlError: null, jsIsLatest: false})
            return
        }
    ...
    tryCompiling(reason, ocaml) {
        try {
            ...
            if (res.js_code) {
                this.setState({reason, ocaml, js: res.js_code, reasonError: null, bsError: null, ocamlError: null, jsIsLatest: true})
        ...
        this.setState({reason, ocaml, reasonError: null, bsError: null, ocamlError: null, jsIsLatest: false})
    }
...
const wrapInExports = code => `(function(exports) {${code}})(window.exports = {})`

const formatOutput = item => item.contents.map(val => JSON.stringify(val)).join(' ')
